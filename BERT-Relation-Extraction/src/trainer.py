...
def train_and_fit(args):
    ...
    #print('The Train Loader is a ',train_loader[0])
    print('Check1')
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        print('Starting Epoch:', epoch,'/', args.num_epochs)
        for i, data in enumerate(train_loader, 0):
            ...
            if (i % update_size) == (update_size - 1):
                ...
                torch.save({
                    ...
                    'best_acc': lm_accuracy_per_batch[-1],\
                    'optimizer' : optimizer.state_dict(),\
                    'scheduler' : scheduler.state_dict(),\
                    'amp': amp.state_dict() if amp is not None else amp
                }, os.path.join("./reldata/" , "test_model_checkpoint_%d.pth.tar" % args.model_no))
                shutil.copy(os.path.join("./reldata/" , "test_model_checkpoint_%d.pth.tar" % args.model_no),
                             os.path.join("gdrive/MyDrive/KnowledgeGraph/SAVE/reldata" , "test_model_checkpoint_%d.pth.tar" % args.model_no))
        ...
        if accuracy_per_epoch[-1] > best_pred:
            ...
            torch.save({
                    ...
                    'scheduler' : scheduler.state_dict(),\
                    'amp': amp.state_dict() if amp is not None else amp
                }, os.path.join("./reldata/" , "test_model_best_%d.pth.tar" % args.model_no))
            shutil.copy(os.path.join("./reldata/" , "test_model_best_%d.pth.tar" % args.model_no),
                             os.path.join("gdrive/MyDrive/KnowledgeGraph/SAVE/reldata" , "test_model_best_%d.pth.tar" % args.model_no))
        ...
        if (epoch % 1) == 0:
            ...
            torch.save({
                    ...
                    'scheduler' : scheduler.state_dict(),\
                    'amp': amp.state_dict() if amp is not None else amp
                }, os.path.join("./reldata/" , "test_checkpoint_%d.pth.tar" % args.model_no))
    ...
    plt.savefig(os.path.join("./" ,"loss_vs_epoch_%d.png" % args.model_no))
    ...
    plt.savefig(os.path.join("./" ,"accuracy_vs_epoch_%d.png" % args.model_no))
