...
def create_pretraining_corpus(raw_text, nlp, window_size=40):
    ...
    entities_of_interest = ["NORP", "FAC", "ORG", "GPE", "LOC", "PRODUCT", "EVENT", \
...
class pretrain_dataset(Dataset):
    def __init__(self, args, D, batch_size=None):
        ...
        tokenizer_path = './%s_tokenizer.pkl' % (model_name)
...
def load_dataloaders(args, max_length=50000):
    ...
    if not os.path.isfile("./D.pkl"):
        ...
        print('Saved the pickle for later!')
        logger.info("Saved pre-training corpus to %s" % "./D.pkl")
    else:
        logger.info("Loaded pre-training data from saved file")
        D = load_pickle("./D.pkl")

    print('We have a dataset of length', len(D))
    #print(D[0:10])
    #random.shuffle(D)
