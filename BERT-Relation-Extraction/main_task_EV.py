...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_data", type=str, default='./SemEval2010_task8_all_data/SemEval2010_task8_training/TRAIN_FILE.TXT', \
                        help="training data .txt file path")
    parser.add_argument("--test_data", type=str, default='./SemEval2010_task8_all_data/SemEval2010_task8_testing_keys/TEST_FILE_FULL.TXT', \
    ...
    if (args.infer == 1) and (args.task != 'fewrel'):
        ...
        file = csv.reader(open('./BERT-Relation-Extraction/econSents.csv',mode='r'))
        relations_file = csv.writer(open('./econrelationships.csv',mode='w'))
        next(file)
        for row in tqdm(file):
            ...
            #print(sent.strip('\n'))
            pred, cat = inferer.infer_sentence(sent.strip('\n'), detect_entities=False)
            
            relations_file.writerow([str(sent),e1,e2,str(pred), str(cat)])
